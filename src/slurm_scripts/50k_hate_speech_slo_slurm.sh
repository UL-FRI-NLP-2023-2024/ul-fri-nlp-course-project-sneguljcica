#!/bin/sh
#SBATCH --job-name=100k_hate_speech_slo        # the title of the job
#SBATCH --output=logs_final/100k_hate_speech_slo.log     # file to which logs are saved
...
srun --nodes=1 --gres=gpu:1 --ntasks=1 --partition=gpu singularity exec --nv ./containers/container-transformers-pytorch-gpu.sif python3 train_code/train_hate_speech_slo.py
